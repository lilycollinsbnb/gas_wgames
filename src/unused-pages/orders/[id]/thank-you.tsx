...
const ThankYou: NextPageWithLayout = () => {
  ...
  useEffect(() => {
    switch (order?.payment_status.toLocaleLowerCase()) {
      case 'payment_pending':
        toast.success(`${t('payment_pending')}`);
        break;

      case 'payment_awaiting_for_approval':
  ...
  useEffect(() => {
    if (!isLoading && order?.payment_gateway) {
      createOrderPayment({
        order_id: query?.id as string,
