...
import type { NextPageWithLayout, OrderedFile, OrderItem } from '@/types'
...
function handleUrl(item: OrderItem) {
...
function OrderedItem({ item }: { item: OrderedFile }) {
  ...
  const { id: order_id } = item.order
  const { id: product_id, shop_id, name, slug, image } = item.product ?? {}
  ...
  const url = handleUrl(item.product)

  return (
    <div className="flex items-start gap-4 border-b border-light-400 py-4 last:border-b-0 dark:border-dark-400 sm:gap-5">
      <AnchorLink href={url}>
        <div className="relative aspect-[4/3] w-28 flex-shrink-0 border border-light-300 dark:border-0 sm:w-32 md:w-36">
          <ImageWithFallback
            alt={name}
            fill
            objectFit="cover"
        ...
        <div className="flex items-center gap-3">
          {getStatus ? (
            <>
              ...
              <Button onClick={() => handleDownload(item.product)}>
            ...
            <Button
              onClick={() =>
                router.push(`${routes.orderUrl(order_id)}/payment`)
              }
            >
          ...
          {!getStatus && (
            <Button
              ...
              onClick={() =>
                openModal('DELETE_ORDER_MODAL', { order_id: order_id })
              }
            >
              {t('text-delete')}
            </Button>
              ...
              <Menu>
                ...
                >
                  <Menu.Items className="absolute top-[110%] z-30 mt-4 w-48 rounded-md bg-light py-1.5 text-dark  shadow-dropdown ltr:right-0 ltr:origin-top-right rtl:left-0 rtl:origin-top-left dark:bg-dark-400 dark:text-light md:top-[78%]">
                    <Menu.Item>
                      <Link
                        href={`${routes.orderUrl(order_id)}/payment`}
