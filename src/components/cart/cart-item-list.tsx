  ...
  return (
    <ul role="list" className={cn('-my-6 w-full', className)}>
      {items.map((item) => {
        ...
        const changedPriceItem = verifiedResponse?.products_with_changed_price?.find(
          (x) => x.id === item.id
        );
        return (
          <li
          ...
          >
            ...
            <CartItem item={item} notAvailable={!!notAvailable} changedPriceItem={changedPriceItem} />
          </li>
        );
      })}
    </ul>
