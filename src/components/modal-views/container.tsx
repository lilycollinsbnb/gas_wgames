...
const GameDownloadView = dynamic(
  () => import('@/components/games/game-download-view')
)

const RedeemBetaAccesView = dynamic(
  () => import('@/components/games/game-redeem-beta-access-view')
)
...
const StripeOneTimePaymentModal = dynamic(
  () => import('@/components/payment/stripe/stripe-one-time-payment-modal'),
  { ssr: false }
)
...
const DeleteOrderModal = dynamic(
  () => import('@/components/orders/order-delete-modal'),
  { ssr: false }
)
...
function renderModalContent(view: MODAL_VIEWS) {
  switch (view) {
    ...
    case 'STRIPE_ONE_TIME_PAYMENT_MODAL':
      return <StripeOneTimePaymentModal />
    case 'DELETE_ORDER_MODAL':
      return <DeleteOrderModal />
    case 'REDEEM_BETA_ACCESS_MODAL':
  ...
  return (
    <Transition appear show={isOpen} as={Fragment}>
      <Dialog
        ...
        dir={dir}
      >
        <div className={`${inter.className} min-h-screen text-center`}>
          <Transition.Child
            as={Fragment}
            ...
            <div className="relative z-50 inline-block min-h-screen w-full transform overflow-hidden text-start align-middle transition-all xs:min-h-[auto] xs:w-auto">
              <div className="relative flex min-h-screen items-center overflow-hidden xs:block xs:min-h-[auto] xs:rounded-md">
                <button
                  ...
                  className="absolute right-4 top-4 z-10 text-dark-900 outline-none transition-all hover:text-dark focus-visible:outline-none 
                  dark:text-dark-800 hover:dark:text-light-200 xs:right-5 xs:top-5 md:right-6 md:top-6 lg:right-7 lg:top-7 3xl:right-7 3xl:top-7"
                >
