...
export default function CategoryFilter({
  ...
  return (
    <section aria-label="Select assets by category" className="app-category-filter-bar sticky top-16 z-20 flex min-h-[64px] w-full overflow-hidden border-b border-light-400 bg-light-100 px-4 py-4 dark:border-dark-300 dark:bg-dark-100 sm:top-[70px] sm:min-h-[70px] sm:px-5 sm:py-5 md:px-6 lg:px-7 3xl:px-8">
      <button
        title={t('text-prev')}
        ref={sliderPrevBtn}
      ...
      <div className="-mb-4 flex items-start overflow-hidden">
        ...
        >
          ...
          {categories
            ...
            .filter((category) => category.is_visible_in_filters)
            .map((category) => (
              <CategoryItem
                id={category.slug}
                key={category.id}
